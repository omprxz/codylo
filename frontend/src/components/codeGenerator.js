...
function Generate() {
  ...
  const [resultsVisib, setResultsVisib] = useState(false)
  const [generatedVisib, setGeneratedVisib] = useState(false)
  ...
  const getCodeFromGemini = async (prompt, model = "gemini-1.5-pro") => {
...
const interval = setInterval(updateLoadingText, 500);
  ...
  <option value="Solidity">Solidity</option>
          ...
          <textarea id="user_prompt" rows="5" placeholder="Describe what you want" className="form-textarea block w-full px-4 py-2 mt-1 rounded-md bg-gray-800 text-white border-none focus:outline-none focus:bg-gray-800 min-h-[120px]" value={user_prompt} onChange={handleUserPrompt}></textarea>
         ...
         { resultsVisib && (<div className="results">
            { generatedVisib && (<h2 className="font-bold text-center text-green-400 text-[1.1rem] mb-0">Generated!</h2>)
            }
            <Code_Block initialCode={code} fileName={filename} language={syntaxLanguage} hasError={errorCode} />
      </div>
    )
      }
    </div> < />
